...
export function EditPartDialog({ part, onSave, onClose }: EditPartDialogProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Convertir valores - PERMITIR CUALQUIER NÚMERO INCLUYENDO 0
    ...
    console.log("📝 FORMULARIO - Valores originales:", {
      precio_texto: formData.precio,
      stock_texto: formData.stock,
      stockMinimo_texto: formData.stockMinimo,
    })

    console.log("🔢 FORMULARIO - Valores convertidos:", {
      precio: precio,
      stock: stock,
      stockMinimo: stockMinimo,
      precio_valido: !isNaN(precio),
      stock_valido: !isNaN(stock),
      stockMinimo_valido: !isNaN(stockMinimo),
    })
    ...
    console.log("🚀 ENVIANDO PARTE ACTUALIZADA:", updatedPart)
  ...
  const handleNumericChange = (field: string, value: string) => {
    // PERMITIR NÚMEROS, PUNTO DECIMAL Y CAMPO VACÍO
    let cleanValue = value

    if (field === "precio") {
      // Para precio: permitir números y punto decimal
      cleanValue = value.replace(/[^0-9.]/g, "")
    } else {
      // Para stock y stockMinimo: solo números enteros
      cleanValue = value.replace(/[^0-9]/g, "")
    }

    console.log(`🔢 Campo numérico ${field}: "${value}" → "${cleanValue}"`)
    ...
    <Dialog open={true} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {/* SECCIÓN DE NÚMEROS - CON CONTROL TOTAL */}
          <div className="bg-blue-50 p-4 rounded-lg border border-blue-200">
            <h4 className="font-medium text-blue-800 mb-3">💰 Control de Inventario</h4>
