...
import { EditarParteDialog } from "@/components/editar-parte-dialog"
...
export function Inventario({ parts, updatePart, deletePart, userRole }: InventarioProps) {
  ...
  const getCategoriaColor = (categoria: string) => {
    const colors = {
      electrica: "bg-yellow-500 text-white",
      motor: "bg-red-500 text-white",
      frenos: "bg-purple-500 text-white",
      suspension: "bg-blue-500 text-white",
      transmision: "bg-green-500 text-white",
    }
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-3xl font-bold text-gray-900">Inventario de Partes</h2>
            ...
            <Card key={part.id} className="hover:shadow-lg transition-shadow">
              <CardHeader className="pb-3">
                <div className="flex items-start justify-between">
                  ...
                  <Badge className={getCategoriaColor(part.categoria)}>{getCategoriaLabel(part.categoria)}</Badge>
                </div>
              </CardHeader>
              <CardContent className="space-y-3">
                <div className="grid grid-cols-2 gap-2 text-sm">
                  ...
                  <div>
                    ...
                    <p className="text-gray-600">
                      ${Number(part.precio).toLocaleString("es-CO", { minimumFractionDigits: 2 })}
                    </p>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <p className="text-gray-600">{part.ubicacion || "Sin ubicación"}</p>
                  </div>
                </div>
                <div>
                ...
                <div>
                  <span className="font-medium text-sm">Modelo Compatible:</span>
                  <p className="text-gray-600 text-sm">{part.modelo_compatible || "No especificado"}</p>
                </div>
                <div>
                  <span className="font-medium text-sm">Proveedor:</span>
                  <p className="text-gray-600 text-sm">{part.proveedor || "No especificado"}</p>
                </div>
      ...
      {filteredParts.length === 0 && (
        <div className="text-center py-12">
          <p className="text-gray-500 text-lg">No se encontraron partes que coincidan con los filtros aplicados</p>
        </div>
      )}
