...
export function ActionHistory({ token }: ActionHistoryProps) {
  ...
  const getActionBadge = (accion: string) => {
    const config = {
      ...
      LOGIN: { label: "LOGIN", className: "bg-purple-500 text-white" },
      LOGOUT: { label: "LOGOUT", className: "bg-gray-500 text-white" },
    }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-3xl font-bold text-gray-900">Historial de Acciones</h2>
                  ...
                  <div key={item.id} className="flex items-start space-x-4 p-4 bg-gray-50 rounded-lg">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between">
                        ...
                        <p className="text-xs text-gray-500">{new Date(item.fecha_accion).toLocaleString()}</p>
                      </div>
                      <p className="text-sm text-gray-600">
                        {item.accion} en {item.tabla_afectada}
